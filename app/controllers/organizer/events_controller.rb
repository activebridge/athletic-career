class Organizer::EventsController < OrganizersController
  expose :organizer, -> { Organizer.find(current_user.organizer_id) }
  expose :events, -> { organizer.events.order(created_at: :desc).page(params[:page]) }
  expose :event
  ...
  def create
    return redirect_to organizer_events_path if event.save
    render :new
  end

  def update
    return redirect_to organizer_events_path if show_event.update(event_params)
    render :edit
  end
